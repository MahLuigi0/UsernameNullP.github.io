...
class Predictor(BasePredictor):
    ...
    def process_audio(self, input_file, chunk_size=5.12, overlap=0.1, seed=None, guidance_scale=3.5, ddim_steps=50, low_pass=False):
        audio, sr = librosa.load(input_file, sr=input_cutoff * 2, mono=False)
        if low_pass:
            audio = lr_filter(audio, input_cutoff, 'lowpass', order=12, sr=sr)
        audio = audio.T
        sr = input_cutoff * 2
    ...
    def predict(self,
        ...
        low_pass: bool = Input(description="Apply low pass filter", default=False)
    ) -> Path:
        if seed == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--low_pass", help="Apply low pass filter", type=bool, required=False, default=False)
